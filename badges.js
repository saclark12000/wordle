(function (global) {
  ...
  const PLAYER_CARD_BADGE_MANIFEST = [
    ...
    },
    {
      ...
      predicate: (ctx) =>
        getMetricNumber(ctx, 'totalGames', 0) >= STEADY_SOLVER_MIN_GAMES &&
        getMetricNumber(ctx, 'solveRate', 0) >= STEADY_SOLVER_UNLOCK_THRESHOLD
    },
    {
    ...
    },
    {
      ...
      collectorEligible: false,
      predicate: (ctx) =>
        getMetricNumber(ctx, 'totalGames', 0) >= FAILED_GAMES_MIN_GAMES &&
        getMetricNumber(ctx, 'failRatio', 0) >= FAILED_GAMES_UNLOCK_THRESHOLD
    },
    {
      id: 'crown_win_ratio',
      icon: (ctx) => buildLayeredBadgeIcon({
      ...
      predicate: (ctx) =>
        getMetricNumber(ctx, 'totalGames', 0) >= CROWN_WIN_RATIO_MIN_GAMES &&
        getMetricNumber(ctx, 'crownRatio', 0) >= 0.3
    },
    {
      id: 'crown_win_streak',
      icon: (ctx) => {
    ...
    },
    {
      ...
      predicate: (ctx) => {
        ...
        return (
          getMetricNumber(ctx, 'crownWins', 0) >= EFFICIENT_CROWNS_MIN_WINS &&
          Number.isFinite(avg) &&
          avg <= EFFICIENT_CROWNS_UNLOCK_MAX_AVG
        );
      }
    },
    {
      ...
      roundBreakdownSlots: (ctx) =>
        getCrownBucketRoundBreakdownSlots(ctx, (bucketKey) => bucketKey === '5' || bucketKey === '6'),
    ...
    },
    {
      ...
      tierInfo: '3 buckets = 0🎲, 4 buckets = 1🎲, 5 buckets = 2🎲, 6 buckets = 3🎲.',
      progress: (ctx) => {
        ...
        return `${formatCountLabel(crownBucketVariety, 'crown bucket')} visited. Tier: ${tierStars}🎲`;
      },
    ...
    },
    {
      ...
      progress: (ctx) =>
        `Fast crowns: ${getFastCrownWins(ctx)}. Late crowns: ${getLateCrownWins(ctx)}.`,
      roundBreakdownSlots: (ctx) =>
        getCrownBucketRoundBreakdownSlots(
          ctx,
  ...
  function evaluateBadges(manifest, ctx, opts = {}) {
    ...
    const earnedBadgeIds = evaluated
      ...
      .filter(Boolean);
    const collectorEligibleBadgeIds = evaluated
      .filter((item) => item.earned && item.entry.collectorEligible !== false)
      .map((item) => item.entry.id)
      .filter((id) => id && id !== 'badge_collector');
    const badgeState = {
      earnedBadgeIds,
  ...
  function summarizeBadgeContextForDebug(ctx, opts = {}) {
    ...
    return {
      ...
      badgeState: {
        ...
        collectorEligibleBadgeIds: Array.isArray(badgeState.collectorEligibleBadgeIds)
          ? [...badgeState.collectorEligibleBadgeIds]
          : []
      },
      metrics: {
